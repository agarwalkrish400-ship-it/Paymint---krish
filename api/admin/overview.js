...
export default async function handler(req, res) {
  ...
  try {
    ...
    `.catch(() => [{ total_detected: 0, total_clicked: 0, total_uploaded: 0 }]);
    ...
    return res.status(200).json({
      ...
      telemetry: {
        total_detected: Number(telemetryCounts?.total_detected || 0),
        total_clicked: Number(telemetryCounts?.total_clicked || 0),
        total_uploaded: Number(telemetryCounts?.total_uploaded || 0),
        conversion_rate: telemetryCounts?.total_detected > 0 
          ? Number(((Number(telemetryCounts.total_uploaded) / Number(telemetryCounts.total_detected)) * 100).toFixed(1)) 
          : 0
      },
